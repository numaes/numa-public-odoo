...
session.web.ActionManager = session.web.OldWidget.extend({
    ...
    ir_actions_act_window: function (action, on_close) {
        ...
        if (action.target === 'new') {
            ...
            if (!action.from_dashboard) {
                this.appendTab($("#tabs"), action.name);
            }
...
session.web.TranslateDialog = session.web.Dialog.extend({
    ...
    on_btn_save: function() {
        ...
        _.each(trads, function(data, code) {
            ...
            if (def_list) {
	            def_list = def_list.pipe(function() {
					return self.view.dataset.write(self.view.datarecord.id, 
	    		                            data, 
		                                    { context : { 'lang': code } },
            	...
            	def_list = self.view.dataset.write(self.view.datarecord.id, 
		                        data, 
		                        { context : { 'lang': code } },
		                        function() {return true},
		                        function() {return true});
