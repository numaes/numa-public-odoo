        ...
        _(datagroups).each(function (group) {
            ...
            if (group.grouped_on) {
                ...
                try {
                    ...
                    $group_column.html(row_data[group.grouped_on].value);
                }
...
var Collection = openerp.web.Class.extend(/** @lends Collection# */{
    ...
    set: function (id, record) {
        $.extend(this._byId[id].attributes, record);
        for (var i=0, length=this.records.length; i<length; i++) {
            var instance_id = this.records[i].get('id');
            if (instance_id === id)
                $.extend(this.records[i].attributes, record);
        }
        return this;
    },
