...
class product_pricelist(report_sxw.rml_parse):
    ...
    def _get_price(self,pricelist_id, product_id,qty):
        ...
        if price_dict[pricelist_id]:
            ...
            res = pool.get('product.product').read(self.cr, self.uid, [product_id], ['price'], context={'pricelist':pricelist_id})
            price =  self.formatLang(res[0]['price'], digits=sale_price_digits)
        return price
