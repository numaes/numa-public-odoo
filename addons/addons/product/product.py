...
class product_uom(osv.osv):
    ...
    def _compute_factor_inv(self, factor):
        return factor and (factor < 1.0 and (1.0 / factor) or factor) or 0.0
    ...
    def _factor_inv_write(self, cursor, user, id, name, value, arg, context=None):
        return self.write(cursor, user, id, {'factor': value}, context=context)
    ...
    def _compute_qty_obj(self, cr, uid, from_unit, qty, to_unit, context=None):
        ...
        elif from_unit.uom_type == 'smaller':
            amount = qty / from_unit.factor
        if to_unit and to_unit.uom_type == 'bigger':
            amount = rounding(amount / to_unit.factor, to_unit.rounding)
        elif to_unit and to_unit.uom_type == 'smaller':
            amount = rounding(amount * to_unit.factor, to_unit.rounding)
        return amount
...
class product_product(osv.osv):
    ...
    def unlink(self, cr, uid, ids, context=None):
        ...
        super(product_product, self).unlink(cr, uid, unlink_ids, context=context)
        if unlink_product_tmpl_ids:
...
class product_supplierinfo(osv.osv):
    ...
    _columns = {
        ...
        'product_id' : fields.many2one('product.template', 'Product', ondelete='cascade', select=True),
