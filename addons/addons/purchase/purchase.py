...
class purchase_order(osv.osv):
    ...
    _defaults = {
        ...
        'state': 'draft',
        'name': lambda obj, cr, uid, context: obj.pool.get('ir.sequence').next_by_code(cr, uid, 'purchase.order'),
        'shipped': 0,
...
#        current_name = self.name_get(cr, uid, ids)[0][1]
    ...
    def _prepare_order_picking(self, cr, uid, order, context=None):
        return {
            ...
            'date': order.date_order + ' 12:00:00',
            'type': 'in',
            ...
            if not order_infos:
                ...
                if porder.origin:
                    if not order_infos['origin']:
                        ...
                        oinfos = set(order_infos['origin'].split(','))
                    oinfos.add(porder.origin)
                    order_infos['origin'] = ','.join(list(oinfos))
...
class purchase_order_line(osv.osv):
    ...
    def onchange_product_id(self, cr, uid, ids, pricelist_id, product_id, qty, uom_id,
        ...
        if supplierinfo_ids:
            ...
            supplier_uom = supplierinfo.product_uom or product.uom_po_id
            if supplier_uom.id != uom_id:
                res['warning'] = {'title': _('Warning'), 'message': _('The selected supplier only sells this product by %s') % supplier_uom.name }
            min_qty = product_uom._compute_qty(cr, uid, supplier_uom.id, supplierinfo.min_qty, to_uom_id=uom_id)
            if qty < min_qty: # If the supplier quantity is greater than entered from user, set minimal.
                res['warning'] = {'title': _('Warning'), 'message': _('The selected supplier has a minimal quantity set to %s %s, you should not purchase less.') % (supplierinfo.min_qty, supplier_uom.name)}
                qty = min_qty
...
class procurement_order(osv.osv):
    ...
    def make_po(self, cr, uid, ids, context=None):
        ...
        for procurement in self.browse(cr, uid, ids, context=context):
            ...
            name = seq_obj.next_by_code(cr, uid, 'purchase.order', context={'force_company': procurement.company_id.id}) or _('PO: %s') % procurement.name
            po_vals = {
                'name': name,
