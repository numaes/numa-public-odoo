...
class crm_lead(crm_case, osv.osv):
    ...
    def _merge_data(self, cr, uid, ids, oldest, fields, context=None):
        ...
        def _get_first_not_null(attr):
            if oldest and hasattr(oldest, attr):
    ...
    def _merge_opportunity_history(self, cr, uid, opportunity_id, opportunities, context=None):
        ...
        for opportunity in opportunities:
            ...
            for pc in opportunity.phonecall_ids:
                pc.write({
                        'opportunity_id': opportunity_id,
                }, context=context)
                
            for m in opportunity.meeting_ids:
                m.write({
                        'opportunity_id': opportunity_id,
                }, context=context)
    ...
    def _merge_opportunity_attachments(self, cr, uid, opportunity_id, opportunities, context=None):
        ...
        first_attachments_names = {x.name:1 for x in first_attachments}
        for opportunity in opportunities:
            attachments = _get_attachments(opportunity.id)
            for attachment in attachments:
                if attachment.name in first_attachments_names:
                    while True:
                        values = dict(
                            name = "%s (%s)" % (attachment.name, first_attachments_names[attachment.name],),
                            res_id = opportunity_id,
                        )
                        first_attachments_names[attachment.name] += 1
                        if values['name'] not in first_attachments_names.keys():
                            break
    ...
    def merge_opportunity(self, cr, uid, ids, context=None):
        ...
        opportunities = self.browse(cr, uid, ids, context=context)
        opportunities_list = list(set(opportunities).union(set(ctx_opportunities)))
        first_opportunity = self._merge_find_oldest(cr, uid, [o.id for o in opportunities_list], context=context)
        tail_opportunities = list(set(opportunities).union(set(ctx_opportunities)))
        tail_opportunities.remove(first_opportunity)
        ...
        data = self._merge_data(cr, uid, [o.id for o in opportunities_list], first_opportunity, fields, context=context)
        ...
        self._merge_notification(cr, uid, first_opportunity, tail_opportunities, context=context)
        #delete tail opportunities
        self.write (cr, uid, [x.id for x in tail_opportunities], {'state': 'done'}, context=context)
    ...
    def _lead_create_partner_address(self, cr, uid, lead, partner_id, context=None):
        ...
        return address.create(cr, uid, {
                    ...
                    'phone': lead.phone,
                    'mobile': lead.mobile,
                    'email': lead.email_from and to_email(lead.email_from) and to_email(lead.email_from)[0],
                    'fax': lead.fax,
