...
class stock_picking(osv.osv):
    ...
    def copy(self, cr, uid, id, default=None, context=None):
        ...
        if ('name' not in default) or (picking_obj.name=='/'):
            seq_obj_name =  'stock.picking.' + default.get('type', picking_obj.type)
    ...
    def action_assign(self, cr, uid, ids, *args):
        ...
        for pick in self.browse(cr, uid, ids):
            ...
            move_ids = [x.id for x in pick.move_lines if x.state in ['assigned', 'done', 'cancel']]
            if len(move_ids) != len(pick.move_lines):
                raise osv.except_osv(_('Warning !'),_('Not all moves are assigned, unable to assign the picking'))
        return True
    ...
    def _prepare_invoice_group(self, cr, uid, picking, partner, invoice, inv_type=None, journal_id=None, context=None):
        ...
        invoice_vals = invoice_obj.default_get(cr, uid, invoice_obj.fields_get(cr, uid, []))
        invoice_vals.update({
        ...
        if inv_type:
            invoice_vals.update(({'type': inv_type}))
        if journal_id:
            invoice_vals.update(({'journal_id': journal_id}))
    ...
    def _prepare_invoice_line(self, cr, uid, group, picking, move_line, invoice_id,
        ...
        if not uos_id and invoice_vals['type'] in ('in_invoice', 'in_refund'):
            uos_id = move_line.product_id.uom_po_id.id
    ...
    def action_invoice_create(self, cr, uid, ids, journal_id=False,
            group=False, type=False, context=None):
        ...
        for picking in self.browse(cr, uid, ids, context=context):
            ...
            if group and partner.id in invoices_group:
                ...
                invoice_vals_group = self._prepare_invoice_group(cr, uid, picking, partner, invoice, inv_type, journal_id, context=context)
            ...
            if new_picking:
                ...
                if pick.type == 'in':
                    if pick.state == 'confirmed':
                        self.force_assign(cr, uid, [new_picking])
                else:
                    if pick.state == 'assigned':
                        self.cancel_assign(cr, uid, [new_picking])
                
            self.action_move(cr, uid, [pick.id])
            wf_service.trg_validate(uid, 'stock.picking', pick.id, 'button_done', cr)
            res[pick.id] = {'delivered_picking': pick.id or False}
...
class stock_move(osv.osv):
    ...
    def onchange_lot_id(self, cr, uid, ids, prodlot_id=False, product_qty=False,
        ...
        """
        if not prodlot_id or not loc_id or not product_id:
            return {}
