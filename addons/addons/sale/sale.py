...
#            qty = qty - qty % q + q
    ...
    def product_id_change(self, cr, uid, ids, pricelist, product, qty=0,
        ...
        if not flag:
            name_context = context_partner.copy()
            del name_context['partner_id']
            result['name'] = self.pool.get('product.product').name_get(cr, uid, [product_obj.id], context=name_context)[0][1]
        domain = {}
        ...
        if not pricelist:
            ...
            price = self.pool.get('product.pricelist').price_get(cr, uid, [pricelist],
                    product, qty or 1.0, None, dict(context,
