...
class res_partner_contact(osv.osv):
    ...
    def _name_get_full(self, cr, uid, ids, field, arg, context=None):
        result = {}
        for rec in self.browse(cr, uid, ids, context=context):
            result[rec.id] = rec.last_name+(rec.first_name and ', '+rec.first_name or '')
        return result

    def _name_set_full(self, cr, uid, ids, prop, value, arg, context=None):
        n = (value or '').split(',')
        last_name = n[0].strip()
        first_name = len(n)>1 and (n[1] or '').strip() or ''
        self.write(cr, uid, ids, {'last_name': last_name, 'first_name': first_name}, context=context)

    def _name_search_full(self, cr, uid, ids, field, arg, context=None):
        n = (value or '').split(',')
        last_name = n[0].strip()
        first_name = len(n)>1 and (n[1] or '').strip() or ''
        if first_name:
            return [('last_name',arg,last_name),('first_name',arg,first_name)]
        else:
            return [('last_name',arg,last_name)]
            
    _columns = {
        'name': fields.function(_name_get_full, fnct_inv=_name_set_full, fnct_search=_name_search_full, 
                                string='Name', size=128, type="char", store=True, select=True),
...
class res_partner_address(osv.osv):
    ...
    def write (self, cr, uid, ids, data, context=None):
        ...
        for pa in self.browse (cr, uid, ids, context=context):
            new_data = data.copy()
            if (not pa.location_id) and (not data.get('location_id', False)):
                loc_id = self.pool.get('res.partner.location').create(cr, uid, {
                    'street': data.get('street',''),
                    'street2': data.get('street2',''),
                    'zip': data.get('zip',''),
                ...
                new_data['location_id'] = loc_id

            super (res_partner_address, self).write (cr, uid, [pa.id], new_data, context=context)
    ...
    def name_get(self, cr, uid, ids, context=None):
        ...
        for rec in self.browse(cr,uid, ids, context=context):
            ...
            if rec.contact_id and rec.contact_id.name:
                components.append(rec.contact_id.name)
                if rec.partner_id and rec.partner_id.name:
                    components.append(rec.partner_id.name)
            if rec.street:
                components.append(rec.street)
            if rec.street2:
                components.append(rec.street2)
            if rec.zip:
                components.append(rec.zip)
            if rec.city:
                components.append(rec.city)
            if rec.state_id and rec.state_id.name:
                components.append(rec.state_id.name)
            if rec.country_id and rec.country_id.name:
                components.append(rec.country_id.name)
            result[rec.id] = components and ", ".join(components) or ''
        return result.items()
