...
class account_fiscalyear_close(osv.osv_memory):
    ...
    def data_save(self, cr, uid, ids, context=None):
        ...
        def _reconcile_fy_closing(cr, uid, ids, context=None):
            """
            This private function manually dreferenceo the reconciliation on the account_move_line given as `ids´, and directly 
        ...
        if move_ids:
            cr.execute("DELETE FROM account_move_line aml USING account_move am, account_period ap, account_fiscalyear af "
                       "       WHERE am.id=aml.move_id AND ap.id=am.period_id AND af.id=ap.fiscalyear_id"
                       "          AND ap.special IS True AND af.id=%d" % period.fiscalyear_id.id)
            cr.execute("DELETE FROM account_move am USING account_period ap, account_fiscalyear af "
                       "       WHERE ap.id=am.period_id AND af.id=ap.fiscalyear_id"
                       "          AND ap.special IS True AND af.id=%d" % period.fiscalyear_id.id)
