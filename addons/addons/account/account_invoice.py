...
class account_invoice(osv.osv):
    ...
    _columns = {
        ...
        'internal_number': fields.char('Invoice Number', size=64, readonly=True, help="Unique number of the invoice, computed automatically when the invoice is created."),
    ...
    def action_date_assign(self, cr, uid, ids, *args):
        for inv in self.browse(cr, uid, ids):
            if inv.date_invoice and not inv.date_due:
            ...
            self.write(cr, uid, [inv.id], {'date_invoice': inv.date_invoice or today, 'date_due': inv.date_due or today})
    ...
    def action_cancel(self, cr, uid, ids, context=None):
        for invoice in self.browse(cr, uid, ids, context=context):
            if invoice.state in ['open', 'paid']:
                if abs(invoice.amount_total) != abs(invoice.residual):
                ...
                invoice.write({'state':'cancel', 'move_id': False})
            self._log_event(cr, uid, ids, -1.0, 'Cancel Invoice %s' % invoice.name)
        return True
...
class account_invoice_line(osv.osv):
    ...
    def product_id_change(self, cr, uid, ids, product, uom, qty=0, name='', type='out_invoice', partner_id=False, fposition_id=False, price_unit=False, address_invoice_id=False, currency_id=False, context=None, company_id=None):
        ...
        result = {}
        # Get product as superuser to get all taxes, then map
        res = self.pool.get('product.product').browse(cr, 1, product, context=context)
        ...
        domain = {}
        result['uos_id'] = type in ['out_invoice', 'out_refund'] and res.uom_id.id or (res.uom_po_id and res.uom_po_id.id) or uom or False
