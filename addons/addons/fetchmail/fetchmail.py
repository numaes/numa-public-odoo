            ...
            if server.type == 'imap':
                try:
                    ...
                    for num in data[0].split():
                        ...
                        res_id = mail_thread.message_process(service_cr, uid, server.object_id.model, data[0][1],
                                                             save_original=server.original,
                                                             strip_attachments=(not server.attach),
                                                             context=context)
                        if res_id and server.action_id:
                            action_pool.run(service_cr, uid, [server.action_id.id], {'active_id': res_id, 'active_ids': [res_id], 'active_model': server.object_id.model})
                        imap_server.store(num, '+FLAGS', '\\Seen')
                        service_cr.commit()
                        count += 1
