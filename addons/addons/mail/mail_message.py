...
class mail_message_common(osv.osv_memory):
    ...
    _columns = {
        ...
        'subtype': fields.char('Message type', size=32, help="Type of message, usually 'html' or 'plain', used to "
                                                             "select plaintext or rich text contents accordingly", readonly=False),
    ...
    def open_document(self, cr, uid, ids, context=None):
        ...
        if ids:
            ...
            if action_ids:
                ...
                for a in ir_act_window.read(cr, uid, action_ids, context=context):
                    o_ids = False
                    if a['domain']:
                        domain = safe_eval(a['domain'], 
                                           locals_dict={
                                               'user': user, 
                                               'uid': uid, 
                                               'time':time, 
                                               ...
                                               'active_ids': [res_id]})
                        selection = [('id','=', res_id)]
                        selection.extend(domain or [])
                        o_ids = o_model.search(cr, uid, selection, context=context)
                    if o_ids or not a['domain']:
                        a.update({
                            'domain' : "[('id','=',%d)]"%(res_id),
                            'nodestroy': True,
                            'context': {}
            ...
            for part in msg_txt.walk():
                ...
                if part.get_content_maintype()=='text':
                    ...
                    if part.get_content_subtype() == 'html':
                        ...
                        #_logger.info("Mail content: [%s]" % content)
                        body = content and len(content) > 2 and tools.ustr(tools.html2plaintext(content)) or ''
                        body = body.replace('&#13;', '')
