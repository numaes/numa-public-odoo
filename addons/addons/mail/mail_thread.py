...
class mail_thread(osv.osv):
    ...
    def message_append(self, cr, uid, threads, subject, body_text=None, email_to=False,
        ...
        for thread in threads:
            ...
            for attachment in attachments:
                ...
                new_name = fname
                count = 1
                while (new_name in already_attached):
                    new_name = fname + '_%d'%count
                    count += 1
                fname = new_name
                already_attached.add(fname)
    ...
    def message_process(self, cr, uid, model, message, custom_values=None,
        ...
        res_id = False
        some_res_id = False
        res_id = tools.res_re.search(msg.get('subject') or '')
        if res_id:
           ...
           if res_id:
               ...
               if model_pool.exists(cr, uid, res_id):
                   ...
                   res_id = False
        if (not res_id) and msg.get('references') or msg.get('in-reply-to'):
            ...
            for ref in references:
                ref = ref.strip()
                ref_groups = tools.reference_re.search(ref)
                if ref_groups:
                    res_id = int(ref_groups.group(1))
                    if ref_groups.group(2) == model and model_pool.exists(cr, uid, res_id):
                        if hasattr(model_pool, 'message_update'):
                            some_res_id = True
                            model_pool.message_update(cr, uid, [res_id], msg, {}, context=context)
                    else:
        ...
        if not some_res_id:
            model_pool.message_new(cr, uid, msg, custom_values, context=context)
