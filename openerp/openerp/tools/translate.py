...
class GettextAlias(object):
    ...
    def __call__(self, source):
        ...
        try:
            ...
            next_frame = frame
            cr = None
            while next_frame and not lang:
                lang = self._get_lang(next_frame)
                if not cr:
                    cr, is_new_cr = self._get_cr(next_frame)
            ...
            if lang:
                if cr:
                    ...
                    res = pool.get('ir.translation')._get_source(cr, 1, None, ('field','selection','code','sql_constraint'), lang, source)
                else:
