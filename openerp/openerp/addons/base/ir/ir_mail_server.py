...
class ir_mail_server(osv.osv):
    ...
    def send_email(self, cr, uid, message, mail_server_id=None, smtp_server=None, smtp_port=None,
        ...
        """
        smtp_from =  message['From'] or message['Return-Path']
        ...
        if mail_server_id:
            ...
            mail_server_ids = self.search(cr, uid, [('smtp_user','=',smtp_from)], limit=1, context=context)
            if not mail_server_ids:
                mail_server_ids = self.search(cr, uid, [], order='sequence', limit=1, context=context)
            if mail_server_ids:
