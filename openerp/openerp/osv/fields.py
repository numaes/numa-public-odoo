...
class many2one(_column):
    ...
    def get(self, cr, obj, ids, name, user=None, context=None, values=None):
        ...
        val_set = set()
        for x in res.values():
            if x and isinstance(x, (int, long)):
                val_set.add(x)
            elif x and isinstance(x, tuple):
                val_set.add(x[0])
...
class one2many(_column):
    ...
    def set(self, cr, obj, id, field, values, user=None, context=None):
        ...
        if self._context:
            context.update(self._context)
        #NUMA disable optimization to avoid a blocked storing of res_partner_address on res.partner creation. TODO: no consequences analyzed
...
class property(function):
    ...
    def _fnct_search(self, tobj, cr, uid, obj=None, name=None, domain=None, context=None):
        prop = tobj.pool.get('ir.property')

        property_field = obj._all_columns.get(name).column
        property_destination_obj = tobj.pool[property_field._obj] if property_field._type == 'many2one' else False
        if property_destination_obj:
            if domain:
                target = "%s,%d" % (property_field._obj, domain[0][2])

                domain = [('name', '=', name), ('value_reference', '=', target)]
                p_ids = prop.search(cr, uid, domain, context=context)
                if p_ids:
                    return [('id', 'in', [x.res_id.id for x in prop.browse(cr, uid, p_ids, context=context)])]
                else:
                    return [('id', '=', 0)]
        return []
    ...
    def __init__(self, obj_prop, **args):
        ...
        function.__init__(self, self._fnct_read, False, self._fnct_write,
                          obj_prop, fnct_search=self._fnct_search, multi='properties', **args)
