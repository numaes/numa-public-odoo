...
def is_leaf(element, internal=False):
    ...
    """
    INTERNAL_OPS = TERM_OPERATORS + ('inselect','not exists', 'not inselect')
...
class expression(object):
                    ...
                    if right is not False:
                        ...
                        if not ids2:
                            if operator in ['like','ilike','in']:
                                ...
                                call_null = False
                                self.__exp[i] = ('id', 'not exists', '(SELECT %s from %s where %s = %s.id)' %
                                                                (id2, field_obj._table, id2, table._table))
                        else:
                    ...
                    if right is not False:
                        ...
                        if not res_ids:
                            if operator in ['like','ilike','in']:
                                ...
                                call_null_m2m = False
                                self.__exp[i] = ('id', 'not exists', '(SELECT %s from %s where %s = %s.id)' %
                                                                (rel_id1, rel_table, rel_id1, table._table))
                            else:
    ...
    def __leaf_to_sql(self, leaf, table):
        ...
        assert operator in (TERM_OPERATORS + ('inselect','not exists', 'not inselect')), \
